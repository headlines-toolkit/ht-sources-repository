  ...
  group('HtSourcesRepository', () {
    ...
    group('updateSource', () {
      test('calls client.updateSource and returns updated source on success',
        ...
        when(() => mockSourcesClient.updateSource(source: updatedSource))
        ...
        verify(() => mockSourcesClient.updateSource(source: updatedSource))
            .called(1);
      });

      test('rethrows SourceNotFoundException from client', () async {
        // Arrange
        when(() => mockSourcesClient.updateSource(source: tSource1))
            .thenThrow(tSourceNotFoundException);
        // Act
        final call = repository.updateSource(source: tSource1);
        // Assert
        await expectLater(call, throwsA(isA<SourceNotFoundException>()));
        verify(() => mockSourcesClient.updateSource(source: tSource1))
            .called(1);
      });
